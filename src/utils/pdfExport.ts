      ...
      for (const block of blocks) {
        ...
        switch (block.type) {
              ...
              try {
                ...
                if (bodymapData.image) {
                  ...
                  imageAttachments.push({
                    ...
                    imageData: bodymapData.image,
                    type: 'bodymap'
                  });
                  valueText = `Schmerzpunkte erfasst (${bodymapData.points?.length || 0} Punkte) - siehe Anhang Seite ${imageAttachments.length}`;
                } else {
          ...
          case 'image':
            if (block.value) {
              // Bild für Anhang sammeln
              const imageData = typeof block.value === 'string' ? block.value : '';
              if (imageData) {
                imageAttachments.push({
                  entryId: entry.id!,
                  entryTitle: `${template?.name || 'Unbekannt'}`,
